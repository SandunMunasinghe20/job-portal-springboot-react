...
const handleMarkAsRead=async()=>{
    try{
        const response = await fetch("http://localhost:8080/api/msg/mark-as-read",{
            method:'POST',
            headers:{
                'Authorization':'Bearer '+token,
                'Content-Type': 'application/json',
    ...
    const getChat = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/msg/chat?senderId=${currentId}&receiverId=${receiverId}`, {
                method: "GET",
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            ...
            toast.success("Chat loaded successfully");
             ...
             data.forEach(msg => {
      if (msg.receiverId === currentId && msg.status !== "READ") {
  ...
  <h3>{chat.length > 0 ? (chat[0].senderId === currentId ? chat[0].receiverName : chat[0].senderName) : "Loading..."}</h3>
</div>
            ...
            <div className="max-w-xl mx-auto mt-6 p-4 border rounded-lg shadow">
                <div className="h-64 overflow-y-auto border-b mb-4 p-2">
                    {chat.map((msg, idx) => (
                        ...
                        <div key={idx} className={`flex flex-col ${msg.senderId == currentId ? 'items-end' : 'items-start'}`}>
                            <div
  ...
  <span>{new Date(msg.sendTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
  ...
  {msg.senderId === currentId && (
    <>
      ...
      {msg.status === "READ" && <MdDoneAll size={16} className="text-blue-500" />}
    </>
  )}
</div>
